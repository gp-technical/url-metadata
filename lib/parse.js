...
module.exports = function (url, body, options) {
	...
	if (!metadata.get('favicon')){
		let favicon = metadata.get('icon-default') || metadata.get('icon-128x128') || metadata.get('icon-192x192') || metadata.get('apple-icon-default') || metadata.get('apple-icon-128x128')

		if (!favicon.startsWith('http') && !favicon.startsWith('//')){
      		let urlData = urlParse.parse(url)
